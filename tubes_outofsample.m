...
for k=1:length(class_list)
    ...
    parfor element_index=1:size(train_set{i},1)
        ...
        round = 1;
        to_remove = [];
        borders_path = strcat('Datasets/', element_index, '/borders_matrix_tube1.mat');
        marc_all = cell(1,1);
        marc_all{1} = marc{1};
        while ~classified
            ...
            [tree_model, marc_tree] = get_trees2(train_set, marc_all, marc_label_par, fmt, 0.2, borders_path, round);
...
%                     output
%                     disp('adding tube 2')
                    borders_path = strcat('Datasets/', element_index, '/borders_matrix_tube1,2.mat');
                    added_2 = true;
                end
...
%                     output
                    if added_2 == true
                        borders_path = strcat('Datasets/', element_index, '/borders_matrix_tube1,2,3.mat');
                    else
                        borders_path = strcat('Datasets/', element_index, '/borders_matrix_tube1,3.mat');
                    end
%                     disp('adding tube 3')
                end
...
%                         disp('nothing else to do')
                    ...
                    borders_path = strcat('Datasets/', element_index, '/borders_matrix_all.mat');
%                     output
                end
            end
%             disp('done')
